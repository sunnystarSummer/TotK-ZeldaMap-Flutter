...
class _MarkerClusterLayerState extends State<MarkerClusterLayer>
  ...
  int _polygonCount = -1;
  final List<PolygonLayer?> _polygonList = [];
  ...
  List<Widget> _buildLayers() {
    ...
    //2023-05-03_Jason
    _polygonList.asMap().forEach((key, value) {
      layers.add(value!);
    });
    ...
    _clusterManager.recursivelyFromTopClusterLevel(
      ...
      if (layer is MarkerNode) {
        ...
        //2023-05-03_Jason
        _showAllPolygon(layer.markers.map((m) => m.point).toList());
      } else {
  ...
  void _showAllPolygon(List<LatLng> points) {
    if (widget.options.isShowAllPolygon) {
      setState(() {
        _polygonList.add(
        ...
        _polygonCount = _polygonList.length;
      });
    }
  }
